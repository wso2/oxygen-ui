...
- Cards open their platform in a new tab by default and lift with an accent
  border on hover, suppressed under `prefers-reduced-motion`
...
- Selecting a card leaves the popover open, since cards open in a new tab; the
  trigger toggles it, and an outside click or `Esc` dismisses it
