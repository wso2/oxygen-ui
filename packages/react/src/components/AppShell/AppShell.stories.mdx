...
import {basicVariationOptions} from '../Header/Header.stories.mdx';
...
<Canvas>
  <Story name="Basic" args={{}}>{Template.bind({})}</Story>
</Canvas>
...
<Canvas>
  <Story name="Gradient" args={{fill: 'gradient'}}>{Template.bind({})}</Story>
</Canvas>
