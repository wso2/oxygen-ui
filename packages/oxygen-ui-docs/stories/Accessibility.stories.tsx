...
const keyboardMap: Array<{ component: string; interaction: string }> = [
  {
    component: 'UserMenu',
    interaction:
      'Enter/Space on the trigger opens the menu with focus inside; Arrow keys move between items; Escape closes and returns focus to the trigger.',
  },
  {
    component: 'ComplexSelect',
    interaction:
      'Standard MUI Select behavior: Enter/Space/Arrow keys open the listbox; Arrow keys navigate options; Enter selects; Escape closes.',
  },
  {
    component: 'AppBreadcrumbs',
    interaction:
      'Tab reaches each crumb and the "…" overflow button; Enter/Space on "…" opens the hidden-crumbs menu (Arrow keys + Escape supported).',
  },
  {
    component: 'Sidebar',
    interaction:
      'Tab/Arrow through items; Enter/Space activates or expands. When collapsed, Enter/Space on a parent opens the nested-items popover.',
  },
  {
    component: 'Header.Toggle',
    interaction:
      'Enter/Space toggles the sidebar; the button exposes aria-expanded for the current state.',
  },
  {
    component: 'NotificationPanel',
    interaction:
      'Named MUI Drawer (default aria-label "Notifications"); focus moves into the panel on open, Escape closes it. Tabs follow Arrow-key navigation. Consumers publish status via liveAnnouncement or useNotificationPanel().setLiveAnnouncement when items arrive (list child-count is not auto-announced).',
  },
  {
    component: 'ListingTable',
    interaction:
      'Sort headers are focusable buttons (Enter/Space toggles sort). The DataGrid variant supports full MUI DataGrid keyboard navigation with a visible focus ring.',
  },
  {
    component: 'SearchBarWithAdvancedFilter',
    interaction:
      'Filter button is labeled and opens an MUI Popover with focus management; Escape closes it.',
  },
]
          ...
          <Typography variant="body2" color="text.secondary" component="div">
            <ul>
              <li>
                ...
                <code>HighContrastTheme</code> provides a maximum-contrast palette for users who
                need it — offer it via <code>ThemeSwitcher</code>.
              </li>
              <li>
                <strong>Known exception:</strong> the brand primary <code>#FF7300</code> in the
                Classic and WSO2 themes does not reach the 4.5:1 ratio required for normal-size
                text. Avoid orange text on light backgrounds for essential content, or use a
                theme/palette override until the tracked palette update lands.
              </li>
              <li>
                Focus indicators are never suppressed by the themes; MUI's{' '}
                <code>:focus-visible</code> styling applies everywhere, and the ListingTable
                DataGrid draws an explicit focus ring.
              </li>
            </ul>
          </Typography>
        </Box>
